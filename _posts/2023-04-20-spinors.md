...
The second term is extremely ambiguous. $g_i^{-1}$ is simply just $L_{g^{-1}*}$. Trouble comes from $dg_i$. Usually one uses $dg$ to denote the pushforward of $g$. But no matter how one interprets $g$ as a map, it isn't consistent with the definition of $\omega_i$. Instead $dg$ actually means $\gamma \in V_p P \cong T_g G$. If I include the implicit annotations, 
...
i.e. derivative of the $G$ fiber "coordinate" of our curve $C(t)=(m(t),g(t))$. **tldr;** $dg = \text{ver} (X)$.
...
This allows us to relate $\sigma_i^* \omega$ to $\sigma_j^* \omega$, which is what we call "Gauge Transformations" on $\mathcal A$.

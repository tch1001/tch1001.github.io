...
- $(h^* \omega)(A^\#) = A$
- $(R_g)^* (h^* \omega) = (\text{Ad}_{g^{-1}})^* (h^*\omega)$
...
Proof of $(h^* \omega)(A^\#) = A$:
Since $A^\# \in V_p P$, the pushforward via $h$ yields $h_* A^\# = (0, \gamma) \in T_m \mathcal U \times T_g G$, where $\gamma$ is 

Theorem: Given a $\mathfrak{g}$-valued one-form $\omega^\mathcal{U}$ on $\mathcal{U}$ and a section $\sigma:\mathcal U \rightarrow \pi^{-1}(\mathcal U)$, there exists a unique connection one-form $\omega$ on $\pi^{-1}(\mathcal U)$ such that $\omega^\mathcal{U} = \sigma^* \omega$.

Construction (without proof): 
$$\omega = (\text{Ad}_{g^{-1}})_* (\pi^* \omega^\mathcal{U}) + \Xi $$
where $g$ is the canonical local trivialization induced by the section $
